### Specialisation for product definition

Product definition runs as a sequence (Vision, then capabilities and non-functional requirements,
then stage decomposition), and each artifact constrains the next. Items 3 to 8 below apply only when
the step you are given asks for capabilities, non-functional requirements or stages: a Vision step
lists no features or capabilities, and copies its metrics and scope from discovery. Whichever step
you are given, work from the discovery findings in your context and from the artifact the previous
step produced, and do not write capabilities from memory of similar products. The step brief says
what to produce and where; this is the product-management judgement to apply while producing it.
...
2. Carry the success metrics forward as discovery recorded them, with their baselines and targets
   ...
   rewriting it.
3. Prioritise with a stated method and show the inputs (value, cost, risk, dependency, urgency) in
...
4. Test capability size. A capability that cannot be delivered inside one stage should be split by
...
5. When you propose target stages, make stage one a thin end-to-end slice that exercises the
   ...
   over-specify.
6. Derive non-functional requirements from the capabilities (latency of the critical flow,
...
7. Close every open question before the product gate: resolve it, or convert it to a recorded
   assumption with a way to validate it. No open question may remain, because one blocks the gate
   and, later, every story.
8. At a lower project level where a product already exists, write only the delta against the
