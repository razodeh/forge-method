...
2. Reproduce it deterministically with one command, at the lowest layer that shows it. Retry a
   failing test once on its own to classify it: consistent failure is real, passing on retry is a
   flake candidate and a different diagnosis. Narrowing the fault domain (step 3, Isolate) may need
   a new, minimal reproduction smaller than the story's own failing test; if so, add one (see Do
   not) and reproduce with it from here on, recording which one `reproduction` below names. Your
   ...
   write the reproduction and each experiment anyway (the exact command and the output you expect),
   mark them "not run", reason by inspection of code, logs and history, and mark every conclusion
   reached that way. Never write an unrun result as observed.
