...
export async function landLane(
  ...
  const { eventStepId, laneStepId, lane, conflictPolicy, checks } = options;
  ...
  await ctx.telemetry.emit({ type: 'MergeStarted', stepId: eventStepId, laneId: lane.laneId });
 ...
 * configured conflict policy and no pre/post checks (those belong to a `merge` step's declared `policy`).
 * Returns the failure when the lane could not be integrated.
 */
