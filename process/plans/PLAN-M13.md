...
| P38 Make the inner loop usable | `build-stage`'s merge policy `preChecks: fast` / `postChecks: full` run as shell commands (`fast: command not found`), so a real merge lands nothing: resolve named check sets to the configured test layers; give auto-integrated lanes the same checks; stack a lane on its sole unmerged predecessor's lane (P19 option (a), taken by the orchestrator) so `implement` sees `generate-tests` and `red`/`green` see `plan`'s record before the merge; point swarm-review perspectives at the reviewed lane. In progress | Q221, Q211 |
...
**M13 closed at `617e549` (2026-09-21) on this boundary; the 21 open owner decisions were delegated to the orchestrator and are recorded in `SPEC-QUESTIONS.md` Q232; the backlog below is `PLAN-M14.md`.**
...
| P42 Briefs and steps for the gate declaration files | P26 defined the evidence files its checks read (`docs/forge/kb/architecture/version-skew.yaml`, `docs/forge/kb/data/migrations.yaml`, `<ENV-id>.dry-run.json` and `<ENV-id>.rollback.json` under `docs/forge/reports/deployments/`) but no shipped brief asks an agent to write them, so G-Integration and G-Deliver need hand-written files or a Waiver on a project that follows the briefs; add the steps/briefs/claims | Q228 |
| P43 Level recording leaves the tree dirty | `intake:record-level` edits the tracked `.forge/config.yaml`, so the next `forge run` refuses until it is committed; options: `forge config set --commit`, or `forge run` ignoring a file only FORGE changed. Also spec text for `--answers` and the elicit `choices` field (proposed in Q227) | Q227 |
| P44 Gate follow-ups | seven module `*.check.yaml` files attach to no gate; `gate check`/`approve` write no `GateReport` artifact (`10` §10.3 rule 4); an agent-run `forge gate approve` cannot be refused (`may_approve` binds the engine, not a shell; owner call); waiver expiry has no cap and `--owner` is free text | Q229 |
| P45 Loader-level agent output validation and stale agents | `forge agent validate` accepts any output type, schema or path (the registry invariant lives only in a repo test); `forge upgrade` leaves stale materialised agents and does not warn; `Story.owner_role` and `Story.interfaces` gaps | Q224 |
